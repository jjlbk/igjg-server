...
const getNewsFromYongin = async (startPage, endPage) => {
    ...
    for (const response of responses) {
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            // html, img
            data.html = $2("#contentsTable > tbody > tr:nth-child(4) > td > div.txt").text();
...
const getNewsFromChangwon = async (startPage, endPage) => {
    ...
    for (const response of responses) {
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            if ($2(imgTag).attr("src") != undefined) {
                data.img =
                    "https://www.changwon.go.kr" + $2(imgTag).attr("src");
            }
...
module.exports = { getNewsFromSuwon, getNewsFromYongin, getNewsFromGoyang, getNewsFromChangwon };